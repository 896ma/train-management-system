<?php
    ...
    session_start();
    $result = $conn->query("SELECT * FROM trains") or die($conn->error);
        ...
        <nav>
          ...
          <div class="card mt-2 ml-2 mr-2">
            ...
            <div class="card-body pt-0 pb-0">
                <a class='btn btn-info mt-1' style='position: absolute; margin-left: 90%;' data-toggle="modal" data-target="#createTrainModal">
                    <strong>Create</strong>
                </a>

                <!-- Create Train -->
                <div class="modal fade"  id="createTrainModal" tabindex="-1" role="dialog" aria-labelledby="createTrainModalTitle" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="createTrainModalTitle" style="color:black">
                                    Create New Train
                                </h5>
                            ...
                            <div class="modal-body">
                                <form action="includes/train.inc.php" method='POST'>
                                    <input type="text" name="trainCode" placeholder="ENTER TRAIN CODE" required>
                                    <input type="text" name="trainName" placeholder="ENTER TRAIN NAME" required>
                                    <input type="number" name='firstClassSeat' placeholder='ENTER FIRST CLASS SEAT CAPACITY' min='0' required>
                                    <input type="number" name='secondClassSeat' placeholder='ENTER SECOND CLASS SEAT CAPACITY' min='0' required>
                ...
                <div class="table-responsive py-5">
                    <table class="table table-bordered table-hover" style='cursor: pointer'>
                        ...
                        while($data = $result->fetch_assoc()){
                            echo"
                                <tbody>
                                    <tr>
                                        ...
                                        <th>{$data['trainCode']}</th>
                                        <th>{$data['trainName']}</th>
                                        <th>
                                            <span class='text-muted'>First Class:</span> {$data['firstClass']}
                                            <hr class='p-0 m-0'>
                                            <span class='text-muted'>Economy:</span> {$data['secondClass']}
                                        </th>
                                        <th>
                                            <div class='row'>
                                                <div class='col-md ml-3'>
                                                    <a type='submit' name='submit' data-toggle='modal' data-target='#updateTrainModal{$data['id']}'>
                                                ...
                                                <div class='col-md'>
                                                    <a type='submit' name='submit' data-toggle='modal' data-target='#deleteTrainModal{$data['id']}''>
                            ...
                            echo"<div class='modal fade' id='updateTrainModal{$data['id']}' tabindex='-1' role='dialog' aria-labelledby='deleteTrainModalTitle' aria-hidden='true'>
                                    <div class='modal-dialog' role='document'>
                                        <div class='modal-content'>
                                            <div class='modal-header'>
                                                <h5 class='modal-title' id='updateTrainModalTitle' style='color:black'>
                                                    Update {$data['trainName']}
                                                </h5>
                                                <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                                                <span aria-hidden='true' style='color:black'>&times;</span>
                                                </button>
                                            ...
                                            <div class='modal-body'>
                                                <form action='includes/update_train.inc.php' method='POST'>
                                                    <input type='hidden' name='trainId' value='{$data['id']}' required>
                                                    <input type='text' name='trainCode' value='{$data['trainCode']}' required>
                                                    <input type='text' name='trainName' value='{$data['trainName']}' required>
                                                    <input type='number' name='firstClassSeat' value='{$data['firstClass']}' min='0' required>
                                                    <input type='number' name='secondClassSeat' value='{$data['secondClass']}' min='0' required>
                                                    <div class='modal-footer mt-4'>
                            ...
                            echo"<div class='modal fade' id='deleteTrainModal{$data['id']}' tabindex='-1' role='dialog' aria-labelledby='deleteTrainModalTitle' aria-hidden='true'>
                                    <div class='modal-dialog' role='document'>
                                        <div class='modal-content'>
                                            <div class='modal-header'>
                                                <h5 class='modal-title' id='deleteTrainModalTitle' style='color:black'>
                                                    Delete {$data['trainName']}
                                                </h5>
                                                <button type='button' class='close' data-dismiss='modal' aria-label='Close'>
                                                <span aria-hidden='true' style='color:black'>&times;</span>
                                                </button>
                                            ...
                                            <div class='modal-footer'>
                                                <a href='includes/delete_train.inc.php?id={$data['id']}' class='btn btn-danger'>Delete</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
